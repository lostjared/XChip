...
void unknown_opcode(Cpu* const _cpu)
{
	...
	auto offs = (get_arr_size(_cpu->memory) - sizeof(bool*)) - 1;
	/* write true to our error flag, IF the element behind it is 255 */
	if(_cpu->memory[offs - 1] == 255)
		*((bool*&)_cpu->memory[offs]) = true;
}
